...
const CUISINES = [
  ...
  { name: 'British', flag: '🇬🇧' },
  { name: 'American', flag: '🇺🇸' },
  { name: 'Middle Eastern', flag: '🥙' } // 💡 Upgraded flag to a Middle Eastern pocket pita emoji!
];
  ...
  return (
    <>
      ...
      >
        ...
        <nav className="flex-1 flex flex-row md:flex-col gap-1 md:gap-1.5 justify-around md:justify-start items-center md:items-stretch w-full overflow-hidden">

          {/* All Recipes (Home) */}
          <button
            onClick={handleAllCuisinesClick}
            className={`flex flex-col md:flex-row items-center gap-1 md:gap-3 px-3 md:px-4 py-1 md:py-2.5 rounded-xl text-[10px] md:text-sm font-bold transition-all duration-300 cursor-pointer w-full md:w-auto ${isHomeActive
          ...
          <button
            ...
            className={`flex flex-col md:flex-row items-center gap-1 md:gap-3 px-3 md:px-4 py-1 md:py-2.5 rounded-xl text-[10px] md:text-sm font-bold transition-all duration-300 cursor-pointer w-full md:w-auto ${isReelsActive
                ? 'text-rose-500 md:bg-rose-500 md:text-white md:shadow-lg md:shadow-rose-500/20'
                : 'text-zinc-400 dark:text-zinc-500 md:text-zinc-500 md:dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800/60'
              }`}
          >
          ...
          <button
            ...
            className={`flex flex-col md:flex-row items-center gap-1 md:gap-3 px-3 md:px-4 py-1 md:py-2.5 rounded-xl text-[10px] md:text-sm font-bold transition-all duration-300 cursor-pointer w-full md:w-auto ${isCreateActive
                ? 'text-rose-500 md:bg-rose-500 md:text-white md:shadow-lg md:shadow-rose-500/20'
                : 'text-zinc-400 dark:text-zinc-500 md:text-zinc-500 md:dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800/60'
              }`}
          >
          ...
          <div className="hidden md:block my-4 border-t border-zinc-200 dark:border-zinc-800 shrink-0" />

          {/* 🔎 Scrollable Cuisines Sub-list (Desktop Only) */}
          <div className="hidden md:flex flex-col gap-1.5 w-full flex-1 overflow-hidden">
            <div className="px-4 py-2 flex items-center gap-2 text-zinc-400 text-[10px] font-bold uppercase tracking-wider shrink-0">
              <Globe className="w-3.5 h-3.5" />
              <span>Explore Cuisines</span>
            </div>

            {/* 
              This scroll container expands to take up remaining height ('flex-1') 
              but is forced to stay within limits and scroll vertically ('overflow-y-auto').
              We also hid the ugly default scrollbar but left it scrollable!
            */}
            <div className="flex-1 overflow-y-auto pr-1 space-y-1.5 [scrollbar-thin] [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-zinc-200/50 dark:[&::-webkit-scrollbar-thumb]:bg-zinc-800/50 [&::-webkit-scrollbar-thumb]:rounded-full">
              {CUISINES.map((cuisine) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-4 py-2 rounded-xl text-xs font-bold transition-all duration-300 cursor-pointer ${isActive
        ...
        <div className="hidden md:block pt-4 border-t border-zinc-200 dark:border-zinc-800 shrink-0">
          <button
            onClick={toggleTheme}
