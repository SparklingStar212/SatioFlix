...
import type { SurvivalPlanResponse, Video } from '../../services/api';
...
const findSynergyMatch = (mealTitle: string, videos: Video[]): Video | undefined => {
  ...
  // We track these separately to completely avoid the TypeScript "never" error
    ...
    <div className="max-w-2xl mx-auto p-6 bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-xl space-y-6 animate-fade-in">
      ...
      <div className="flex bg-zinc-100 dark:bg-zinc-800 p-1 rounded-xl">
        ...
        >
          ...
          <span>Meal Timetable ({plan.meals.length})</span>
        </button>
        <button
      ...
      {activeTab === 'meals' && (
        <div className="space-y-4 max-h-[500px] overflow-y-auto pr-1 scrollbar-thin">
          {plan.meals.map((meal, idx) => {
            ...
            return (
              <div key={idx} className="p-4 bg-zinc-50 dark:bg-zinc-800/40 rounded-xl border border-zinc-200 dark:border-zinc-700/60 flex flex-col sm:flex-row gap-4 justify-between relative overflow-hidden">

                {matchedVideo && <div className="absolute top-0 right-0 w-16 h-16 bg-rose-500/5 rounded-bl-full -z-0" />}

                <div className="z-10 flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="px-2.5 py-1 bg-zinc-200 dark:bg-zinc-700 text-zinc-700 dark:text-zinc-300 rounded-lg text-[10px] font-black uppercase">
                      Day {meal.day}
                    </span>
                    <span className="text-xs font-bold text-zinc-500">
                      ~{plan.currency} {meal.totalEstimatedCost}
                    </span>
                  </div>
